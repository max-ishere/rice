...
class Behaviour:
    ...
    hide_input: str = (
        False  # Specify a character to hide with or True for default character
    )
        ...
        conf.update(
            {
                ...
                "hidden-character": c
                if type(c := maybe(lambda: self.behaviour.hide_input)) is str
                else None,
                ...
                "history-file": h
                if type(h := maybe(lambda: self.behaviour.history)) == str
                else None,
            }
        )

        conf = "\n".join(
            [f"{key} = {value}" for (key, value) in conf.items() if value is not None]
        )

        conf += "\n".join([f"include = {path}" for path in self.include or []])

        return (
            "\n".join(["# " + line for line in self.header.text.splitlines()])
            + "\n\n"
            + conf
        )
...
def main():
    from rice.blocks.tofi_apple_theme import AppleTofi
